...
async function AcceptApplication(message, responseData, applicationId) {
    ...
    let applicationData = await con.awaitQuery("SELECT applicationName, applicationDenyMessage, applicationReviewRole FROM applications WHERE applicationID = ? AND guildid = ?", [applicationId, message.guild.id]);
    ...
    if (applicationData.applicationDenyMessage != null && applicationData.applicationDenyMessage != "") {
        ...
        DenyMessage.setDescription(`Your application for ${applicationData.applicationName} in the server ${message.guild.name} has been denied.`)
    }

    member.send(DenyMessage).catch(c => {
    ...
    message.channel.send(DenyServer)
